...
class RegisterPR(CreateView):
    ...
    model = PR
    fields = ['tgl_pr', 'partnumber', 'partname', 'qty', 'uom', 'deskripsi','status']
    success_url = reverse_lazy('list_pr')
    def get(self, request):
        return render(request, 'crud_pr/register.html')

    def post(self, request):
        # Ambil data dari form yang dikirim sebagai list
        tanggal_pr_list = request.POST.getlist('tanggal_pr[]')
        part_number_list = request.POST.getlist('part_number[]')
        part_name_list = request.POST.getlist('part_name[]')
        qty_list = request.POST.getlist('qty[]')
        uom_list = request.POST.getlist('uom[]')
        deskripsi_list = request.POST.getlist('deskripsi[]')
        status_list = request.POST.getlist('status[]')

        pr_objects = []
        for i in range(len(tanggal_pr_list)):
            pr_objects.append(PR(
                tgl_pr=tanggal_pr_list[i],
                partnumber=part_number_list[i],
                partname=part_name_list[i],
                qty=qty_list[i],
                uom=uom_list[i],
                deskripsi=deskripsi_list[i] if deskripsi_list[i] else None,
                status=status_list[i] if status_list[i] else 'Sent to Purchase'
            ))

        PR.objects.bulk_create(pr_objects)  # Simpan banyak data sekaligus
        status_choices = StatusPR.STATUS_CHOICES
        return render(request, "register_pr.html", {"status_choices": status_choices})
        #messages.success(request, "Data Purchase Request berhasil disimpan!")
        return redirect('list_pr')  # Redirect ke halaman daftar PR
