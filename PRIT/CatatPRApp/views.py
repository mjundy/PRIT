...
from .forms import JenisPartForm, PRForm, StatusPRForm
...
#Class untuk Jenis Part

class ListJenisPart(ListView):
    template_name = 'crud_jenis_part/tables.html'
    model = JenisPart
    context_object_name = "list_jenis_part"
    #distinct_jenis = ListProxy.values_list('jenispart', flat=True).distinct()

class RegisterJenisPart(CreateView):
    template_name = 'crud_jenis_part/register.html'
    model = JenisPart
    form_class = JenisPartForm
    success_url = reverse_lazy('list_jenis_part')

class UpdateJenisPart(UpdateView):
    template_name = 'crud_jenis_part/register.html'
    model = JenisPart
    form_class = JenisPartForm
    success_url = reverse_lazy('list_jenis_part')
